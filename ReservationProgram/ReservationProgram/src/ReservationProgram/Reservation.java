...
public class Reservation {
	...
	ArrayList<boolean[]> tableReservableData = new ArrayList();
	...
	private void saveReservation() {
		...
		int chosenTableNum = tableData3.get(chooseTable()).tableNum;
		...
		reservationData1.AddReservation(userAccountIndex, time, intValue, chosenTableNum, reservationNumber, status,restaurantIndex ,orderedMenu );
	}
	...
	private void PrintTime() {
		...
		this.reservationData2 = this.reservationData.list;
		for(String s : timeSet) {
			...
			for(int i=0;i<reservationData2.size();i++) {
				if(!reservationData2.get(i).status.equals("예정"))
					continue;
				if(reservationData2.get(i).restaurantIndex != this.restaurantIndex) {
					continue;
				}
				...
				if(Math.abs(duration.getSeconds())<=3600) {
					...
					for(TableData td : tableData3) {
						System.out.println(reservationData2.get(i).tableNum);
			...
			if(input.equals("c")) {
				...
				if(inputInt>=1&&inputInt<=restaurantDataList.size()) {
					if(restaurantDataList.get(inputInt-1).menuInfo.menuName.length>=1) {
						...
						tableData.LoadDataFile("Table_"+restaurantDataList.get(restaurantIndex).restaurantName + ".txt");
						tableData2 = tableData.getTableData();
						break;
					}
					else {
	...
	private void ReserveMenu() {
		while(true) {
			...
			if(input.equals("c")) {
				...
				try {
					OrderedMenuInfo orderedMenuTemp = restaurantData.StringToOrderedMenuInfo(input);
					if(orderedMenuTemp.menuIndex[orderedMenuTemp.menuIndex.length-1]>restaurantData.getRestaurantData().get(restaurantIndex).menuInfo.menuName.length) {	//입력한 메뉴 인덱스가 범위를 넘어선게 있는지 체크
						System.out.println("[오류] 메뉴 번호 범위 외의 메뉴 번호가 입력되었습니다. 다시 입력해주세요.");
						continue;
					}
					boolean terminate = false;
					for(int orderedAmount : orderedMenuTemp.orderedAmount) {
						if(orderedAmount<=0) {
							terminate = true;
							break;
						}
					...
					if(terminate) {
						System.out.println("[오류] 메뉴 수량은 1 이상의 값이어야 합니다. 다시 입력해주세요.");
						continue;
					}
					orderedMenu = orderedMenuTemp;
				} catch (ParseException e) {
					// TODO Auto-generated catch block
